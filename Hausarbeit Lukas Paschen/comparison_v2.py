...
def run_readability(htmlstring):
    ...
    try:
        doc = readability.Document(htmlstring)
        return doc.summary()
...
def evaluate_result(result, item):
    ...
    for to_include in item['with']:
        to_include_as_bytes=str.encode(to_include)
        if len(to_include) == 0:
            print(item)
        if result is not None:
            if to_include in result:
            #if to_include in result.encode('utf-8'):
            #if to_include.encode('utf-8') in result:
            #if to_include.decode('utf-8') in result:
                true_positives += 1
            else:
                false_negatives += 1
        else:
...
def bo3_newspaper():
    """runs fulltext from the newspaper module 3 times and returns the best result;
    the best result is the one with the highest fscore"""
    best_fscore=0
    best_result=()
    for i in range(3):
        ...
        current_result=calculate_scores(newspaper_result)
        current_fscore=calculate_scores(newspaper_result)[3]
        if current_fscore>best_fscore:
            best_fscore=current_fscore
            best_result=current_result
        # clean the dict before next run:
        newspaper_result['true positives']=0
        newspaper_result['false positives']=0
        newspaper_result['true negatives']=0
        newspaper_result['false negatives']=0
        newspaper_result['time']=0
    return best_result
    
def bo3_trafilatura():
    """runs trafilatura 3 times and returns the best result;
    the best result is the one with the highest fscore"""
    best_fscore=0
    best_result=()
    for i in range(3):
        ...
        current_result=calculate_scores(trafilatura_result)
        current_fscore=calculate_scores(trafilatura_result)[3]
        if current_fscore>best_fscore:
            best_fscore=current_fscore
            best_result=current_result
        # clean the dict before next run:
        trafilatura_result['true positives']=0
        trafilatura_result['false positives']=0
        trafilatura_result['true negatives']=0
        trafilatura_result['false negatives']=0
        trafilatura_result['time']=0
    return best_result
    
def bo3_trafilatura_fallback():
    """runs trafilatura with fallback 3 times and returns the best result;
    the best result is the one with the highest fscore"""
    best_fscore=0
    best_result=()
    for i in range(3):
        ...
        current_result=calculate_scores(trafilatura_X_result)
        current_fscore=calculate_scores(trafilatura_X_result)[3]
        if current_fscore>best_fscore:
            best_fscore=current_fscore
            best_result=current_result
        # clean the dict before next run:
        trafilatura_X_result['true positives']=0
        trafilatura_X_result['false positives']=0
        trafilatura_X_result['true negatives']=0
        trafilatura_X_result['false negatives']=0
        trafilatura_X_result['time']=0
    return best_result
    
def bo3_justext():
    """runs justext 3 times and returns the best result;
    the best result is the one with the highest fscore"""
    best_fscore=0
    best_result=()
    for i in range(3):
        ...
        current_result=calculate_scores(justext_result)
        current_fscore=calculate_scores(justext_result)[3]
        if current_fscore>best_fscore:
            best_fscore=current_fscore
            best_result=current_result
        # clean the dict before next run:
        justext_result['true positives']=0
        justext_result['false positives']=0
        justext_result['true negatives']=0
        justext_result['false negatives']=0
        justext_result['time']=0
    return best_result
    
def bo3_inscriptis():
    """runs inscriptis 3 times and returns the best result;
    the best result is the one with the highest fscore"""
    best_fscore=0
    best_result=()
    for i in range(3):
        ...
        current_result=calculate_scores(inscriptis_result)
        current_fscore=calculate_scores(inscriptis_result)[3]
        if current_fscore>best_fscore:
            best_fscore=current_fscore
            best_result=current_result
        # clean the dict before next run:
        inscriptis_result['true positives']=0
        inscriptis_result['false positives']=0
        inscriptis_result['true negatives']=0
        inscriptis_result['false negatives']=0
        inscriptis_result['time']=0
    return best_result
    
def bo3_html2text():
    """runs html2text 3 times and returns the best result;
    the best result is the one with the highest fscore"""
    best_fscore=0
    best_result=()
    for i in range(3):
        ...
        current_result=calculate_scores(html2text_result)
        current_fscore=calculate_scores(html2text_result)[3]
        if current_fscore>best_fscore:
            best_fscore=current_fscore
            best_result=current_result
        # clean the dict before next run:
        html2text_result['true positives']=0
        html2text_result['false positives']=0
        html2text_result['true negatives']=0
        html2text_result['false negatives']=0
        html2text_result['time']=0
    return best_result
    
def bo3_readability():
    """runs readability 3 times and returns the best result;
    the best result is the one with the highest fscore"""
    best_fscore=0
    best_result=()
    for i in range(3):
        ...
        current_result=calculate_scores(readability_result)
        current_fscore=calculate_scores(readability_result)[3]
        if current_fscore>best_fscore:
            best_fscore=current_fscore
            best_result=current_result
        # clean the dict before next run:
        readability_result['true positives']=0
        readability_result['false positives']=0
        readability_result['true negatives']=0
        readability_result['false negatives']=0
        readability_result['time']=0
    return best_result
...
#everything, nothing, trafilatura_result, justext_result, trafilatura_X_result, goose_result, readability_result, inscriptis_result, newspaper_result, html2text_result, boilerpipe_result, newsplease_result, jparser_result = {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}
#everything, nothing, trafilatura_result, justext_result, trafilatura_X_result, goose_result, readability_result, inscriptis_result, newspaper_result, html2text_result, boilerpipe_result, newsplease_result = {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}
#everything, nothing, trafilatura_result, justext_result, trafilatura_X_result, goose_result, readability_result, inscriptis_result, newspaper_result, html2text_result, boilerpipe_result= {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}
#everything, nothing, trafilatura_result, justext_result, trafilatura_X_result, goose_result, readability_result, inscriptis_result, newspaper_result, html2text_result= {}, {}, {}, {}, {}, {}, {}, {}, {}, {}
everything, nothing, trafilatura_result, justext_result, trafilatura_X_result, readability_result, inscriptis_result, newspaper_result, html2text_result= {}, {}, {}, {}, {}, {}, {}, {}, {}
...
best_readability_result=bo3_readability()
print("Precision: ",best_readability_result[0])
print("Recall: ",best_readability_result[1])
print("Accuracy: ",best_readability_result[2])
print("F-Score: ",best_readability_result[3])

print("Running html2text...")
best_html2text_result=bo3_html2text()
print("Precision: ",best_html2text_result[0])
print("Recall: ",best_html2text_result[1])
print("Accuracy: ",best_html2text_result[2])
print("F-Score: ",best_html2text_result[3])

print("Running inscriptis...")
best_inscriptis_result=bo3_inscriptis()
print("Precision: ",best_inscriptis_result[0])
print("Recall: ",best_inscriptis_result[1])
print("Accuracy: ",best_inscriptis_result[2])
print("F-Score: ",best_inscriptis_result[3])

print("Running trafilatura...")
best_trafilatura_result=bo3_trafilatura()
print("Precision: ",best_trafilatura_result[0])
print("Recall: ",best_trafilatura_result[1])
print("Accuracy: ",best_trafilatura_result[2])
print("F-Score: ",best_trafilatura_result[3])

print("Running trafilatura with fallback algorithm...")
best_trafilatura_X_result=bo3_trafilatura_fallback()
print("Precision: ",best_trafilatura_X_result[0])
print("Recall: ",best_trafilatura_X_result[1])
print("Accuracy: ",best_trafilatura_X_result[2])
print("F-Score: ",best_trafilatura_X_result[3])

print("Running the 'fulltext' algorithm from the newspaper module...")
best_newspaper_result=bo3_newspaper()
print("Precision: ",best_newspaper_result[0])
print("Recall: ",best_newspaper_result[1])
print("Accuracy: ",best_newspaper_result[2])
print("F-Score: ",best_newspaper_result[3])

print("Running justext...")
best_justext_result=bo3_justext()
print("Precision: ",best_justext_result[0])
print("Recall: ",best_justext_result[1])
print("Accuracy: ",best_justext_result[2])
print("F-Score: ",best_justext_result[3])
